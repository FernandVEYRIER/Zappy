...
public class DictionaryDrawer : PropertyDrawer  {
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        if (expanded)
        {
            ...
            if (GUI.Button(contentPosition, "+"))
            {
                keys.arraySize++;
                values.arraySize++;
                setProperty(keys.GetArrayElementAtIndex(size), null);
                setProperty(values.GetArrayElementAtIndex(size), null);
            }
            contentPosition = GetNextPos(ref position);
            contentPosition.width = 230;
            contentPosition.x = (position.width + 14) / 2f - contentPosition.width / 2f;
    ...
    public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
    {
        if (property.isExpanded)
            return base.GetPropertyHeight(property, label) * (size + 6);  // assuming original is one row
        else
